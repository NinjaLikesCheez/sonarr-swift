...
struct ImportListRequestsTests {
	...
	private static func sonarrImportPreset() async throws -> ImportListResource {
		let schemas = try await client.request(.importListSchema)
		return try #require(schemas.first(where: { $0.implementation == "SonarrImport" }))
	}
	...
	func test_addImportList_importLists_importList_updateImportList_deleteImportList() async throws {
		let preset = try await Self.sonarrImportPreset()
