...
struct ImportListRequestsTests {
	...
	@Test func updateImportListRequestConstruction() throws {
		...
		let prepared = request.prepare(URLRequest(url: URL(string: "http://localhost:8989/api/v3/importlist/1")!))
		let components = URLComponents(url: prepared.url!, resolvingAgainstBaseURL: false)
		#expect(components?.queryItems == [URLQueryItem(name: "forceSave", value: "true")])
	...
	@Test func updateImportListsRequestConstruction() throws {
		let bulkResource = ImportListBulkResource(ids: [1, 2, 3], tags: [4], applyTags: .add, qualityProfileId: 2)
		...
		#expect(decoded.qualityProfileId == 2)
	}
	...
	@Test func performImportListActionRequestConstruction() throws {
		let request = SonarrRequest.performImportListAction(name: "getUserLists", sampleImportList)

		#expect(request.method == .post)
		#expect(request.path == "api/v3/importlist/action/getUserLists")
	...
	@Test func importListResourceDecoding() throws {
		...
		#expect(importList.name == "Trakt List")
		#expect(importList.implementation == "TraktUserImport")
